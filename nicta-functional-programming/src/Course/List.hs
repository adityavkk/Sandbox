...
-- or map f = foldLeft (\ acc x -> acc ++ f x :. Nil) Nil
...
-- or filter f = foldRight (\x -> if f x then (x:.) else id) Nil
...
seqOptional = foldRight f (Full Nil)
  where f x acc = if isMt x || isMt acc then Empty else Full (fromFull x :. fromFull acc)
...
find ::
  ...
  -> List a
  -> Optional a
find f = safeHead . filter f

safeHead (x:.xs) = Full x
safeHead Nil = Empty
