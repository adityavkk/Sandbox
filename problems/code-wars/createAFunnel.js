...
const dfs = (vertex, graph) => {
  ...
  const go = vertex => {
    ...
    unvisitedNeighbours.forEach(v => visited.add(v))

    return unvisitedNeighbours.length + unvisitedNeighbours.reduce((sum, v) => sum + go(v), 0)
  }
  return go(vertex)
}
...
class Funnel {
  ...
  drip() {
    ...
    const getHeaviestNeighbour = vertex => {
      ...
      if (neighbours.size == 1) return Array.from(neighbours)[0]
      const [leftWeight, rightWeight] = Array.from(neighbours).map(n => graph.weightAbove(n))
    ...
    const replaceWithHeaviestNeighbour = vertex => {
      ...
      const graph = this.#funnel
      const neighboursOfVertex = graph.neighbours(vertex)
      const heaviest = getHeaviestNeighbour(vertex)
      console.log(neighboursOfVertex)
      const otherNeighbours = Array.from(neighboursOfVertex).filter(v => v !== heaviest)
      graph.removeEdge(vertex, heaviest)
      otherNeighbours.forEach(n => graph.addEdge(heaviest, n))
      replaceWithHeaviestNeighbour(heaviest)
      return heaviest
    }

    graph.root = replaceWithHeaviestNeighbour(graph.root)
